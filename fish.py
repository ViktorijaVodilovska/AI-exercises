...
if __name__ == '__main__':
    ...
    crit = input()
    test = input().split(" ")
    test = [test[i] for i in range(len(test)) if i != col_index]

    train_set = dataset[:int(0.85 * len(dataset))]
    train_x = [[t[i] for i in range(len(t)-1) if i != col_index] for t in train_set]
    train_y = [t[-1] for t in train_set]

    test_set = dataset[int(0.85 * len(dataset)):]
    test_x = [[t[i] for i in range(len(t)-1) if i != col_index] for t in test_set]
    ...
    print(classifier.predict_proba([test])[0])
